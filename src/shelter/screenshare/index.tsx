...
export async function onLoad() {
    ...
    window.legcord.screenshare.getSources(
        async (_event: Event, sources: IPCSources[]) => {
            var audioSources: Node[] | undefined;
            if (window.legcord.platform === "linux") {
                let venmic = await window.legcord.screenshare.venmicList();
                if (venmic.ok) {
                    audioSources = venmic.targets;
                    console.log(
                        `Venmic audio source targets: ${audioSources.map((node) => node["node.name"])}`,
                    );
                } else {
                    console.log(
                        "Venmic is NOT OK. Venmic will not be available for screensharing with audio.",
                    );
                }
            }
